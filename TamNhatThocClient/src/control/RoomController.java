	...
	private void showPopup(MouseEvent event) {
	    ...
	    if (event.getSource() instanceof ImageView) {
	        ...
	        if (userIndex >= 0 && userIndex < u.size()) {
	            ...
	            // Khởi tạo bộ đếm thời gian để trì hoãn hiển thị popup 1 giây
	            popupDelay = new PauseTransition(Duration.seconds(1));
	            popupDelay.setOnFinished(e -> popup.show(avatar, event.getScreenX() + 10, event.getScreenY() + 10));
	            popupDelay.playFromStart(); // Bắt đầu đếm thời gian
	        }
	    }
	}
